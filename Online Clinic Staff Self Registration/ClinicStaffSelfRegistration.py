...
#1. Registering New Clinic Staff in LunarHCPPortalV1


FilePath = str(pathlib.Path().resolve()) + r'\Excel Files\UrlsForProject.xlsx'
Sheet = 'Portal Urls'
...
FilePath = str(pathlib.Path().resolve()) + '\Excel Files\ClinicStaffRegistrationFromPortal.xlsx'

Seconds = 300/1000
Sheet = 'CS Registration Page Data'
...
for RowIndex in range(2, RowCount + 1):
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 8)) != 'None'):
        ClinicTextBox.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 8))
    else:
        ClinicTextBox.click()
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 9)) != 'None'):
        PhoneTextBox.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 9))
    else:
        PhoneTextBox.click()
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 10)) != 'None'):
        CityTextBox.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 10))
    else:
        CityTextBox.click()
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 11)) != 'None'):
        ...
        SelectStateDropDownList.select_by_visible_text(RWDE.ReadData(FilePath, Sheet, RowIndex, 11))
    else:
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 12)) != 'None'):
        ZipCodeTextBox.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 12))
    else:
    ...
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 13)) != 'None'):
        FaxTextBox.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 13))
    else:
        FaxTextBox.click()

    #time.sleep(Seconds)
    #if RWDE.ReadData(FilePath, Sheet, RowIndex, 14) == 'S':
    ...
    if(ErrorMsg != ' Loading Loading'):
        if (ErrorMsg != '' ):
            ...
            if (ErrorElementAvailable == True):
                RequiredErrorMsg = BEP.WebElement(driver, EC.presence_of_element_located, By.XPATH, '//div[3]/div/div/div[3]/div[1]/div', 60)
                ErrorMsg += ' ' + RequiredErrorMsg.text
                RWDE.WriteData(FilePath, Sheet, RowIndex, 15, 'Hold')
                RWDE.WriteData(FilePath, Sheet, RowIndex, 16, ErrorMsg)
    else:
        RWDE.WriteData(FilePath, Sheet, RowIndex, 15, 'Passed')
